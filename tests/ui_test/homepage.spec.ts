...
test.skip('Verify homepage url and homepage content', async ({ page }) => {
    ...
    await expect(page.locator('p.text-lg.text-white.text-center.max-w-lg.mb-8')).toHaveText(
        'Keep track of your spending, manage your budget, and achieve your financial goals with ease.'
    );
    await expect(page.locator('p.text-lg.text-white.text-center.max-w-lg.mb-8')).toBeVisible();
    await expect(page.locator('p.text-lg.text-white.text-center.max-w-lg.mb-8')).toHaveCSS(
        'color',
        'rgb(255, 255, 255)'
    );
    ...
    await expect(page.locator('a.py-2.text-white\\/50.hover\\:text-white.ml-auto.mr-10')).toHaveAttribute(
        'href',
        '/auth'
    );
    await expect(page.locator('a.py-2.text-white\\/50.hover\\:text-white.ml-auto.mr-10')).toHaveText(
        'Log In / Sign In'
    );
});
